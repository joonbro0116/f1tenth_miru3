...
def main():
    ...
    ap.add_argument("--pgm", required=True, help="map .pgm")
    ...
    ap.add_argument("--outdir", default="bounds_out", help="CSV/디버그 출력 폴더")
    ...
    ap.add_argument("--save_world", action="store_true", help="월드좌표 CSV도 저장")
    ...
    cv2.imwrite(os.path.join(args.outdir, "bounds_debug.png"), dbg)

    # CSV 저장 (픽셀/월드)
    save_csv_pixels(os.path.join(args.outdir, "outer_bound_pixels.csv"), outer_s)
    save_csv_pixels(os.path.join(args.outdir, "inner_bound_pixels.csv"), all_inner)
    if args.save_world:
        outer_w = pixels_to_world(outer_s, H, resolution, origin)
        inner_w = pixels_to_world(all_inner, H, resolution, origin)
        save_csv_world(os.path.join(args.outdir, "outer_bound_world.csv"), outer_w)
        save_csv_world(os.path.join(args.outdir, "inner_bound_world.csv"), inner_w)


    print(f"[OK] 저장 완료 -> {args.outdir}/outer_bound_pixels.csv, {args.outdir}/inner_bound_pixels.csv")
    if args.save_world:
        print(f"[OK] 월드좌표도 저장 -> outer_bound_world.csv, inner_bound_world.csv")
