...
# GUI Map file selection
def select_map_file_gui():
    """Open GUI file dialog to select a map CSV file"""
    # Create a root window but hide it
    root = tk.Tk()
    root.withdraw()
    
    # Set initial directory to tracks folder
    tracks_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "inputs", "tracks")
    
    # If tracks directory doesn't exist, use current directory
    if not os.path.exists(tracks_dir):
        tracks_dir = os.path.dirname(os.path.abspath(__file__))
    
    # Open file dialog
    file_path = filedialog.askopenfilename(
        title="Select Track Map File",
    ...
    if file_path:
        ...
        return map_name, file_path
    else:
        # User cancelled, use default
        print("No file selected. Using default map: e7_floor5_square")
        default_path = os.path.join(tracks_dir, "e7_floor5_square.csv")
        if os.path.exists(default_path):
            return "e7_floor5_square", default_path
        else:
            print("Default map not found!")
            return None, None

# Create the parser with optional arguments for automation
parser = argparse.ArgumentParser(description='Generate optimal trajectory for F1TENTH racing.')
parser.add_argument('--map_name', type=str, default='', help='Name of the map (skip interactive selection)')
parser.add_argument('--map_path', type=str, default='', help='Path to the map centerline (should be a .csv)')
parser.add_argument('--export_path', type=str, default='', help='Path to copy from the filepath in the /outputs')
parser.add_argument('--auto', action='store_true', help='Use default map without interactive selection')
...
if args.map_name:
    ...
    # Use default map without interaction
    MAP_NAME = 'e7_floor5_square'
    MAP_PATH = ''
else:
    # GUI file selection
    MAP_NAME, MAP_PATH = select_map_file_gui()
    if MAP_NAME is None:
