...
def plot(image, title, plot_mode):
    if plot_mode > 0:
        fig = plt.figure()
        plt.title(title)  # Use plt.title instead of set_window_title
...
def denoise(image, depth, plot_mode):
    ...
    # 더 보수적인 디노이즈: 원래 파라미터로 복원
    radius = 3
    db_scan = DBSCAN(eps=radius, min_samples=max(1, pixelsInCircle(radius)//4))
    ...
    # 원래보다 조금 더 관대한 접근
    min_cluster_size = max(20, len(points) // 50)  # 전체 포인트의 2% 이상 또는 최소 20개
    ...
    # 두 번째 DBSCAN
    db_scan = DBSCAN(eps=2.0, min_samples=1)
...
def prune(image, plot_mode):
    ...
    print(f"Debug: Non-zero pixels in skeletonized image: {non_zero_pixels}")
    ...
    print(f"Debug: Total edges added to graph: {edge_count}")
    
    g.findCycle()
    
    print(f"Debug: Cycle found with {len(g.cycle)} points")
...
if __name__=='__main__':
    ...
    parser.add_argument('--subsample_period', help='defines the subsampling rate of the center-line pixels for the output points', nargs='?', type=int, default=3)
    ...
    # Check if track_cycle is empty
    if len(track_cycle) == 0:
        print("Error: No track cycle found! The pruning step failed to find a closed cycle.")
        print("This might be due to:")
        print("1. Too aggressive denoising parameters")
        print("2. Disconnected track segments after skeletonization") 
        print("3. No closed loop in the track")
        sys.exit("Empty track cycle - processing failed")
    
    # Transform to Real-World Coordinates
    print(f"Transforming to Real-World Coordinates ({len(track_cycle)} points)")
    ...
    output_dir = "/home/f1/f1tenth_ws/path_generate/centerline/"
