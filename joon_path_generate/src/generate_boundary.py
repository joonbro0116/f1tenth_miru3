...
def find_boundary_simple(map_img, center_pixel, direction_vector, resolution, max_distance=8.0):
    """
...
def extract_track_edges_revised(centerline_path, pgm_path, yaml_path, output_path):
    ...
    for i, center_point in enumerate(centerline):
        ...
        left_dist = find_boundary_simple(map_img, center_pixel, left_perp, resolution, max_distance=8.0)
        right_dist = find_boundary_simple(map_img, center_pixel, right_perp, resolution, max_distance=8.0)
...
def visualize_track_edges_revised(results, save_path=None, show_plot=False, map_img=None, resolution=None, origin=None):
    """트랙 경계점들을 원본 맵과 함께 시각화합니다."""
    ...
    # 원본 맵 배경 표시 (제공된 경우)
    if map_img is not None and resolution is not None and origin is not None:
        # 맵 이미지의 월드 좌표 범위 계산
        h, w = map_img.shape
        world_x_min = -origin[0]
        world_x_max = w * resolution - origin[0]
        world_y_min = -origin[1]
        world_y_max = h * resolution - origin[1]
        
        # 맵 이미지를 배경으로 표시
        plt.imshow(map_img, extent=[world_x_min, world_x_max, world_y_min, world_y_max],
                  cmap='gray', alpha=0.7, origin='lower')
        print(f"맵 배경 표시됨: 월드 좌표 범위 X({world_x_min:.2f}, {world_x_max:.2f}), Y({world_y_min:.2f}, {world_y_max:.2f})")
    
    # 센터라인 그리기
    plt.plot(results[:, 0], results[:, 1], 'b-', linewidth=3, label='Centerline', alpha=0.9)
    ...
    for i in range(0, len(results), step_size):
        ...
        if norm > 0:
            ...
            # 경계점 계산 및 표시 (유효한 거리만)
            if 0.1 < left_dist < 8.0:
                left_point = np.array([center_x, center_y]) + left_perp * left_dist
                left_boundary_points.append(left_point)
                plt.plot([center_x, left_point[0]], [center_y, left_point[1]], 'r-', alpha=0.8, linewidth=1.5)
                valid_left_count += 1
            
            if 0.1 < right_dist < 8.0:
                right_point = np.array([center_x, center_y]) + right_perp * right_dist
                right_boundary_points.append(right_point)
                plt.plot([center_x, right_point[0]], [center_y, right_point[1]], 'g-', alpha=0.8, linewidth=1.5)
                valid_right_count += 1
    
    # 경계점들을 연결선으로 표시
    if left_boundary_points:
        left_boundary_points = np.array(left_boundary_points)
        plt.plot(left_boundary_points[:, 0], left_boundary_points[:, 1], 'ro', alpha=0.9, markersize=3, label='Left Boundary', markeredgecolor='darkred')
    
    if right_boundary_points:
        right_boundary_points = np.array(right_boundary_points)
        plt.plot(right_boundary_points[:, 0], right_boundary_points[:, 1], 'go', alpha=0.9, markersize=3, label='Right Boundary', markeredgecolor='darkgreen')
    ...
    plt.title('F1tenth Track: Map with Centerline and Detected Boundaries', fontsize=14, fontweight='bold')
...
def main():
    ...
    # 시각화 수행 (맵 배경과 함께)
    visualize_track_edges_revised(results, plot_path, args.show_plot, map_img, resolution, origin)
